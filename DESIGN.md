...
## 후속: ETF 종류별 탐색 (codex/etf-categories)

네 종류 + 지역/배당 필터로 홈의 7개 혼합 분류를 정리했다. 새 대표 상품 5개를 추가해 총 9개 실제 가격을 수집했다.
기존 /explore/korea, us, dividend, tech, world 해시도 새 분류로 해석한다.
검색이 선택한 카테고리를 벗어나 다른 상품을 보여주던 문제를 공통 filterEtfs로 수정했다.
추가 상품 상세의 설명·상품유형·기초지수를 설정에서 읽고 비교는 자산 차이를 안내한다.
KOSPI200 Lab은 독립 snapshot을 유지한다. 자동화는 두 snapshot을 모두 갱신한 뒤 커밋한다.
검증: Python 17개, 기존 Node 계산 및 metadata 검사, 9 ETF 분류·검색 조합·상세 설명·이종 자산 비교·전 상품 시뮬레이션·기존 Lab 4개 유지 검사 통과.
브라우저 시각 검증은 여전히 미완료이며 Node view 검사는 실제 DOM/모바일 상호작용 검증을 대체하지 않는다.

## 후속: 기본 정보 보완 및 분배금 조회 (codex/etf-distributions)

추가 5개 상품의 검증된 기본 정보 필드를 연결하고 보수 표시를 4자리로 확장했다.
기존 메타데이터 파일에 부분 지급 이력 2개 상품을 추가했다. 출처·단위·중복·날짜 순서·금액을
발행 시 검증하며 브라우저도 잘못된 이력을 거부한다. 상세 표와 카드/비교 요약에서 조회한다.
지급 기준일을 분배락일로 취급하지 않고 기존 가격 시뮬레이터와 분리한다.
미확보 필드, 과거 자료, 전체 이력 미확보를 그대로 표시한다. 새 서버나 의존성은 없다.
