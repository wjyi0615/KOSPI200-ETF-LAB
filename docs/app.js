...
function render(){
  ...
  const base=data.universe[0],basePrices=indices.map(i=>data.prices[base.symbol][i]);
  ...
  document.getElementById('relative-label').textContent=`${base.name} 대비`;
  const years=(Date.parse(period.end)-Date.parse(period.start))/86400000/365.25;
  const baseReturn=period.metrics[base.symbol].cumulative_return;
  document.getElementById('comparison').innerHTML=data.universe.map(etf=>{const m=period.metrics[etf.symbol],relative=(1+m.cumulative_return)/(1+baseReturn)-1;return `<tr><td><a href="${etf.source_url}" target="_blank" rel="noopener noreferrer"><b>${etf.name}</b></a><small>${etf.manager} · ${etf.symbol}</small></td><td>${percent(m.cumulative_return)}</td><td class="${relative>=0?'positive':'negative'}">${percent(relative)}</td><td>${years>=1?percent(m.cagr):'—'}</td><td>${percent(m.annualized_volatility)}</td><td>${m.sharpe_ratio===null?'—':m.sharpe_ratio.toFixed(2)}</td><td>${percent(m.maximum_drawdown)}</td></tr>`}).join('');
  const curves=data.universe.map(etf=>{const p=indices.map(i=>data.prices[etf.symbol][i]);let peak=p[0];return {etf,values:p.map((v,i)=>{peak=Math.max(peak,v);if(state.mode==='price')return v;if(state.mode==='drawdown')return v/peak-1;if(state.mode==='relative')return (v/p[0])/(basePrices[i]/basePrices[0])-1;return v/p[0]-1})}});
  ...
  let svg='<desc id="chart-desc">선택 기간 네 운용사 ETF의 '+({price:'종가',cumulative:'누적수익률',relative:'기준 ETF 대비 상대 성과',drawdown:'낙폭'}[state.mode])+' 비교. 상세 지표는 위 표에 표시됩니다.</desc>';
  for(let i=0;i<5;i++){const v=lo+(hi-lo)*i/4,Y=y(v),label=state.mode==='price'?Math.round(v).toLocaleString():state.mode==='relative'?percent(v):Math.round(v*100)+'%';svg+=`<line x1="80" y1="${Y}" x2="965" y2="${Y}" stroke="#e4ebf0"/><text x="68" y="${Y+5}" text-anchor="end" fill="#587083" font-size="14">${label}</text>`;}
  if(state.mode==='relative'&&lo<0&&hi>0){const zero=y(0);svg+=`<line x1="80" y1="${zero}" x2="965" y2="${zero}" stroke="#8da8b7" stroke-dasharray="6 5"/>`;}
  ...
  document.getElementById('chart-title').textContent={price:'종가 (KRW)',cumulative:'누적수익률',relative:`${base.name} 대비 상대 성과`,drawdown:'기간 고점 대비 낙폭'}[state.mode];
  document.getElementById('chart-note').textContent={price:'ETF별 단위가격 차이는 성과 차이가 아닙니다.',cumulative:'각 ETF의 기준일 가격 = 0%',relative:`${base.name}=0% · 작은 성과 차이를 읽기 쉽게 축 범위를 자동 확대`,drawdown:'선택 기간 내 최고가격에서 얼마나 하락했는가'}[state.mode];
