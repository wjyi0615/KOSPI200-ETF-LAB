...
function compare(){
 ...
 return heading('COMPARE','같은 지수, 다른 선택.','가격은 같은 기간으로, 기본 정보는 표시된 기준일로 확인해요. 순자산 자료 날짜가 달라 현재 규모 순위로 해석하지 마세요.')+dataNotice()+(list.length?`<div class="actions">${list.map(e=>`<button data-select="${e.ticker}" aria-pressed="true">${esc(e.name)} 제외 ×</button>`).join('')}<a href="#/explore">ETF 추가하기</a></div><section class="panel">${periodControls()}<div class="section">${performance(list)}</div></section><div class="table-wrap section"><table><caption>선택한 ETF 비교 · 미확보는 0이 아닙니다</caption><thead><tr><th scope="col">비교 항목</th>${list.map(e=>`<th scope="col"><a href="#/etf/${e.ticker}">${esc(e.name)}</a></th>`).join('')}</tr></thead><tbody>${[
