...
const categories=[['korea','🇰🇷','국내 대표지수'],['us','🇺🇸','미국 대표지수'],['dividend','💰','배당'],['bonds','🏦','채권'],['tech','🤖','AI / 반도체'],['commodity','🥇','금 / 원자재'],['world','🌏','해외시장']];
const terms=[
...
function card(e){return `<article class="card"><span class="badge">${esc(e.benchmark)}</span><h3><a href="#/etf/${e.ticker}">${esc(e.name)}</a></h3><small>${e.ticker} · ${esc(e.issuer)}</small><p class="price">${F.money(e.price)}</p><small>${e.asOf} 종가</small><dl>${metric('최근 1개월',F.percent(e.returns.month))}${metric('거래량',F.number(e.volume),'volume')}</dl><details><summary>보수·규모·분배금 확인</summary><dl>${metric('순자산',F.money(e.aum),'aum')}${metric('총보수',F.percent(e.expenseRatio),'fee')}${metric('분배금','미확보','distribution')}</dl><p class="subtle">확인되지 않은 값은 0으로 표시하지 않습니다.</p></details><div class="actions"><button data-select="${e.ticker}" aria-pressed="${selected.has(e.ticker)}">${selected.has(e.ticker)?'비교에서 제외':'비교에 담기'}</button><a href="#/etf/${e.ticker}">자세히 →</a></div></article>`}
function home(){return `<section class="hero"><div>${heading('YOUR FIRST ETF','투자의 첫걸음,<br>이해하는 것부터.','ETF가 처음이어도 괜찮아요. 무엇에 투자하는지 살펴보고, 비슷한 상품의 차이를 하나씩 알아보세요.')}<div class="actions"><a class="button primary" href="#/explore">ETF 찾아보기 →</a><a class="button" href="#/learn">ETF가 뭔가요?</a></div></div><div class="hero-art" aria-label="학습 순서"><div><small>01 · 무엇을 담고 있나요?</small><strong>투자대상 알아보기</strong></div><div><small>02 · 같은 지수, 다른 ETF</small><strong>차이를 비교하기</strong></div><div><small>03 · 매달 조금씩 투자했다면</small><strong>내 금액으로 계산하기</strong></div></div></section><section><h2>어떤 투자를 하고 싶나요?</h2><p class="muted">지금은 국내 대표지수부터 시작합니다. 새로운 시장은 검증된 데이터를 확보한 뒤 열립니다.</p><div class="grid categories">${categories.map(([id,emoji,label])=>`<a class="card category" href="#/explore/${id}"><span aria-hidden="true">${emoji}</span><h3>${label}</h3><small>${id==='korea'?'KOSPI200 · 4개 ETF':'준비 중 · 상품 미등록'}</small></a>`).join('')}</div></section><section class="section two-col"><article class="panel"><span class="badge">첫 번째 연구실</span><h2>같은 KOSPI200,<br>무엇이 다를까요?</h2><p>가격 흐름과 위험을 같은 기간으로 맞춰 살펴보세요.</p><a href="#/kospi200">기존 ETF Lab 열기 →</a></article><article class="panel"><span class="badge">직접 해보기</span><h2>매달 30만원씩<br>투자했다면?</h2><p>누적 납입금과 평가금액을 함께 보며 과거 투자 경로를 이해해요.</p><a href="#/simulator">시뮬레이터 시작 →</a></article></section>`}
function explore(category='korea'){
 const cat=categories.find(c=>c[0]===category);
 if(!cat)return empty('등록되지 않은 카테고리입니다.');
 const list=etfs.filter(e=>e.category===category);
 return heading('EXPLORE',cat[2],'가격보다 먼저, 어떤 지수를 따라가는지 확인해 보세요.')+`<div class="actions">${categories.map(([id,,name])=>`<a class="button" href="#/explore/${id}" ${id===category?'aria-current="page"':''}>${name}</a>`).join('')}</div>`+(category==='korea'?dataNotice():'')+(list.length?`<label class="search">이름·종목코드 검색<input id="search" type="search" placeholder="예: KODEX 또는 069500"></label><div class="grid" id="cards">${list.map(card).join('')}</div><div class="actions"><a class="button primary" href="#/compare">선택한 ETF 비교 →</a><span class="muted">최대 3개까지 담을 수 있어요.</span></div>`:empty('이 카테고리의 ETF는 준비 중입니다.','실제 데이터가 확보되면 상품이 표시됩니다.'));
}
function empty(title,note=''){return `<div class="empty"><h2>${title}</h2><p>${note}</p><a href="#/explore">국내 ETF 둘러보기 →</a></div>`}
...
function compare(){
 ...
 return heading('COMPARE','같은 지수, 다른 선택.','최대 3개의 ETF를 같은 기준으로 비교해요.')+dataNotice()+(list.length?`<div class="actions">${list.map(e=>`<button data-select="${e.ticker}" aria-pressed="true">${esc(e.name)} 제외 ×</button>`).join('')}<a href="#/explore">ETF 추가하기</a></div><section class="panel">${periodControls()}<div class="section">${performance(list)}</div></section><div class="table-wrap section"><table><caption>선택한 ETF 비교 · 미확보는 0이 아닙니다</caption><thead><tr><th scope="col">비교 항목</th>${list.map(e=>`<th scope="col"><a href="#/etf/${e.ticker}">${esc(e.name)}</a></th>`).join('')}</tr></thead><tbody>${[
 ['운용사',e=>esc(e.issuer)],['추종지수',e=>esc(e.benchmark)],['상장일',e=>e.inceptionDate||'미확보'],['순자산',e=>F.money(e.aum),'aum'],['거래량',e=>F.number(e.volume),'volume'],['총보수',e=>F.percent(e.expenseRatio),'fee'],['추적오차',e=>F.percent(e.trackingError),'tracking'],['괴리율',e=>F.percent(e.premiumDiscount),'premium'],['분배금',()=> '미확보','distribution'],['1개월 가격 수익률',e=>F.percent(e.returns.month)],['3개월 가격 수익률',e=>F.percent(e.returns.quarter)],['1년 가격 수익률',e=>F.percent(e.returns.year)]
 ].map(([label,value,key])=>`<tr><th scope="row">${label}${key?help(key,label):''}</th>${list.map(e=>`<td>${value(e)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`:empty('비교할 ETF를 먼저 담아 주세요.','탐색 화면에서 최대 3개를 선택할 수 있어요.'))+`<section class="panel section"><h2>같은 KOSPI200인데 왜 다를까요?</h2><p>따라가는 지수는 같아도 보수, 운용 방식, 분배금 지급 시점과 거래가격은 다를 수 있어요. 가격선이 조금 다르다고 곧바로 운용 능력 차이라고 해석하지 않아요.</p><p>① 같은 기간인가요? ② 분배금 기준이 같나요? ③ 시장가격과 NAV를 구분했나요?</p><p class="muted">현재는 종가만 비교합니다. NAV·지수 데이터가 없으므로 추적오차를 계산하지 않습니다.</p><a href="#/learn/tracking">추적오차와 성과 차이 알아보기 →</a></section>`;
}
function detail(ticker){
 const e=etfs.find(e=>e.ticker===ticker);if(!e)return empty('해당 ETF를 찾을 수 없습니다.');
 return heading('ETF OVERVIEW',esc(e.name),`${e.ticker} · ${esc(e.issuer)} · ${esc(e.benchmark)}`)+`<p class="notice">국내 대표 주가지수 KOSPI200을 따라가는 ETF입니다. 한 종목으로 여러 기업에 분산 투자하지만 가격 하락 위험은 남아요.</p>`+dataNotice()+`<div class="stats"><div class="stat">마지막 종가<strong>${F.money(e.price)}</strong><small>${e.asOf}</small></div><div class="stat">1년 가격 수익률<strong>${F.percent(e.returns.year)}</strong></div><div class="stat">총보수 ${help('fee','총보수')}<strong>${F.percent(e.expenseRatio)}</strong></div></div><div class="actions"><button data-select="${e.ticker}" aria-pressed="${selected.has(e.ticker)}">${selected.has(e.ticker)?'비교에서 제외':'비교에 담기'}</button><a class="button primary" href="#/simulator/${e.ticker}">이 ETF로 계산하기</a><a href="${safeUrl(e.sourceUrl)}" target="_blank" rel="noopener noreferrer">상품 정보 출처 ↗</a></div><section class="panel"><h2>ETF 한눈에 보기</h2><dl>${metric('추종지수',esc(e.benchmark),'index')}${metric('운용사',esc(e.issuer))}${metric('유형','국내 주식형 · 패시브','passive')}${metric('순자산',F.money(e.aum),'aum')}${metric('거래량',F.number(e.volume),'volume')}${metric('상장일',e.inceptionDate||'미확보')}</dl></section><section class="panel section"><h2>가격은 어떻게 변했을까요?</h2>${periodControls()}${performance([e])}<p class="muted">KOSPI200 지수와 NAV 시계열은 아직 확보하지 않아 표시하지 않습니다.</p></section><section class="panel section"><h2>이 ETF는 어디에 투자하고 있을까?</h2>${e.holdings.length?holdingsChart(e.holdings):'<p class="notice">구성종목과 비중 데이터 준비 중</p><p>기준일이 있는 공식 구성종목 데이터를 연결하면 상위 종목의 비중을 여기에 표시합니다. 익숙한 기업 이름이나 현재 비중을 과거에 소급해 넣지 않습니다.</p>'}</section><section class="panel section"><h2>가격 수익률과 분배금 재투자</h2><p>지금 차트는 공급자 종가 기준입니다. 분배금을 현금으로 모으거나 재투자한 결과와는 구분해야 해요.</p><button disabled>실제 분배금 재투자 · 데이터 검증 대기</button><p class="subtle">분배락일·지급일·1좌당 분배금·가격 조정 이력을 확보해야 중복 계산 없이 제공할 수 있습니다.</p><a href="#/learn/reinvest">재투자는 어떤 뜻일까요? →</a></section><section class="panel section"><h2>오늘 이 ETF가 오른 이유</h2><p class="notice">수익률 기여도 분석 준비 중</p><p>전 거래일 종목 비중 × 같은 기간 종목 수익률로 가격 기여도를 근사할 예정입니다. 배당·현금·보수·리밸런싱 등으로 ETF 실제 수익률과 차이가 생깁니다.</p><p class="muted">현재는 전일 비중과 종목별 가격이 없어 상승 이유를 추정하지 않습니다.</p></section>`;
}
...
function filterCards(query){const q=query.trim().toLowerCase();const list=etfs.filter(e=>(e.name+e.ticker+e.issuer).toLowerCase().includes(q));document.getElementById('cards').innerHTML=list.length?list.map(card).join(''):'<p class="empty">검색 결과가 없습니다.</p>';}
...
root.addEventListener('change',event=>{if(event.target.closest('#simulation'))document.getElementById('sim-result').innerHTML='<p class="empty">입력이 변경되었습니다. 다시 계산해 주세요.</p>';});
