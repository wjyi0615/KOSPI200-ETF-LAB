...
function fundamental(e,key){
 ...
 const value=key==='aum'?F.number(Math.round(r.value/1e8))+'억원':key==='expenseRatio'?'연 '+(r.value*100).toFixed(4)+'%':esc(r.value);
...
/** Show confirmed payments without implying complete history or a yield. */
function distributionSummary(e){
 const last=e.distributions.at(-1);
 return last?`1좌당 ${F.money(last.amountPerShare)}<small style="display:block">확인 내역 중 최근 · ${last.paymentDate} 지급 · 세전</small>`:'미확보';
}
function distributionSection(e){
 const h=e.distributionHistory;
 if(!h)return '<section class="panel section"><h2>분배금 지급 내역</h2><p class="notice">분배금 자료 미확보</p><p>자료가 없다는 뜻이며, 분배금을 지급하지 않았다는 뜻은 아닙니다.</p></section>';
 return `<section class="panel section"><h2>분배금 지급 내역</h2><p>${distributionSummary(e)}</p><p class="notice">확인한 일부 내역입니다. 전체 지급 이력이나 연간 분배율이 아닙니다. 지급 기준일은 분배락일과 다르며, 다음 분배금은 확정되지 않았습니다.</p><p class="subtle">수록 지급 기준일 ${e.distributions[0].recordDate} ~ ${e.distributions.at(-1).recordDate} · 확인 ${h.checkedAt} · <a href="${safeUrl(h.sourceUrl)}" target="_blank" rel="noopener noreferrer">${esc(h.sourceName)} ↗</a></p><div class="table-wrap"><table><caption>${esc(e.name)} 확인된 세전 분배금</caption><thead><tr><th scope="col">지급 기준일</th><th scope="col">실제 지급일</th><th scope="col">1좌당 금액</th></tr></thead><tbody>${[...e.distributions].reverse().map(x=>`<tr><td>${x.recordDate}</td><td>${x.paymentDate}</td><td>${F.money(x.amountPerShare)}</td></tr>`).join('')}</tbody></table></div><p class="subtle">이 내역은 가격 차트와 투자 계산기에 합산하지 않습니다. 재투자 계산에는 분배락일과 가격 조정 기준의 추가 검증이 필요합니다.</p></section>`;
}
...
function compare(){
 ...
 ['운용사',e=>esc(e.issuer)],['추종지수',e=>esc(e.benchmark)],['상장일',e=>fundamental(e,'inceptionDate')],['순자산',e=>fundamental(e,'aum'),'aum'],['거래량',e=>F.number(e.volume)+(e.volume!==null?'좌 · '+e.asOf:''),'volume'],['총보수',e=>fundamental(e,'expenseRatio'),'fee'],['추적오차',e=>F.percent(e.trackingError),'tracking'],['괴리율',e=>F.percent(e.premiumDiscount),'premium'],['확인한 분배금',e=>distributionSummary(e),'distribution'],['1개월 가격 수익률',e=>F.percent(e.returns.month)],['3개월 가격 수익률',e=>F.percent(e.returns.quarter)],['1년 가격 수익률',e=>F.percent(e.returns.year)]
